...
<div id="main">
    ...
    <section>
        <article><h1>Massive.js 3.0: A Postgres-centric Data Access Tool</h1><p><a href="https://travis-ci.org/dmfay/massive-js"><img src="https://travis-ci.org/dmfay/massive-js.svg?branch=master" alt="Build Status"></a></p>
...
<!-- vim-markdown-toc -->
<h2>Installation</h2><pre class="prettyprint source"><code>npm i massive --save</code></pre><p><strong>Starting with version 3, Massive uses Promises exclusively. If you need a callback-based API, download version 2.x from <a href="https://github.com/dmfay/massive-js/releases">the Releases page</a>.</strong></p>
...
}</code></pre><h4>Querying</h4><p><strong>findOne</strong> finds a single object with a primary key or a criteria object.</p>
...
});</code></pre><p><code>modify</code> adds and updates fields in an existing document (or any JSON/JSONB column) <em>without</em> replacing the entire body. Fields not defined in the <code>changes</code> object are not modified.</p>
...
});</code></pre><p>Like <code>run</code>, prepared statements in script files can use named parameters instead of <code>$1</code>-style indexed parameters. Named parameters are formatted <code>${name}</code>. Other delimiters besides braces are supported; consult the pg-promise documentation for a full accounting.</p>
...
<pre class="prettyprint source lang-js"><code>db.tests.find({priority: 'low'}, {stream: true}).then(stream => {
  const tests = [];
...
<p>Documentation for Massive.js 2.x is at <a href="http://massive-js.readthedocs.org/en/latest/">readthedocs</a>.</p>
