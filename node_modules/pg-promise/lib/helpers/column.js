...
function parseColumn(name) {
    const m = name.match(/\??[a-zA-Z0-9$_]+(\^|~|#|:raw|:alias|:name|:json|:csv|:value)?/);
    if (m && m[0] === name) {
        const res = {};
        if (name[0] === '?') {
            res.cnd = true;
        ...
        const mod = name.match(/\^|~|#|:raw|:alias|:name|:json|:csv|:value/);
        if (mod) {
...
function isValidMod(mod) {
    const values = ['^', '~', '#', ':raw', ':alias', ':name', ':json', ':csv', ':value'];
    return values.indexOf(mod) !== -1;
}

function isValidVariable(name) {
    const m = name.match(/^[0-9]+|[a-zA-Z0-9$_]+/);
    return !!m && m[0] === name;
}
